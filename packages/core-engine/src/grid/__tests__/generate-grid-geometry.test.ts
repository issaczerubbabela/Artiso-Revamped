...
import { generateGridGeometry, MAX_GRID_CELLS } from '../generate-grid-geometry';
import { GridConfigError } from '../types';

const baseConfig = { rows: 4, cols: 4, numberingMode: 'off' as const };

describe('generateGridGeometry', () => {
  it('generates (cols - 1) vertical and (rows - 1) horizontal lines', () => {
    const { lines } = generateGridGeometry(400, 400, { ...baseConfig, rows: 4, cols: 5 });
  ...
  it('spaces lines evenly across image-space dimensions', () => {
    const { lines } = generateGridGeometry(100, 200, { rows: 2, cols: 2, numberingMode: 'off' });
  ...
  it('produces a single cell with no lines for a 1x1 grid', () => {
    const { lines } = generateGridGeometry(100, 100, { rows: 1, cols: 1, numberingMode: 'off' });
    expect(lines).toHaveLength(0);
  });

  it('includes labels when numberingMode is not off', () => {
    const { labels } = generateGridGeometry(100, 100, { rows: 2, cols: 2, numberingMode: 'numbers' });
    expect(labels).toHaveLength(4);
  });

  it('omits labels when numberingMode is off', () => {
    const { labels } = generateGridGeometry(100, 100, { ...baseConfig, numberingMode: 'off' });
    expect(labels).toHaveLength(0);
  });

  it('rejects a grid below 1x1', () => {
    expect(() => generateGridGeometry(100, 100, { ...baseConfig, rows: 0 })).toThrow(GridConfigError);
  });

  it('rejects a grid exceeding the max cell count', () => {
    expect(() =>
      generateGridGeometry(1000, 1000, { rows: 100, cols: 100, numberingMode: 'off' }),
    ).toThrow(GridConfigError);
  });

  it('accepts a grid exactly at the max cell count', () => {
    expect(() =>
      generateGridGeometry(1000, 1000, { rows: 50, cols: 50, numberingMode: 'off' }),
    ).not.toThrow();
