...
// test file (generate-rectangular-grid.test.ts etc.) -- this only verifies
...
describe('generateGridGeometry dispatch', () => {
  it('rectangular', () => {
    const config: GridConfig = { ...BASE_STYLE, type: 'rectangular', rows: 2, cols: 2, numberingMode: 'off', snapToImage: true };
  ...
  it('radial', () => {
    const config: GridConfig = { ...BASE_STYLE, type: 'radial', centerX: 0.5, centerY: 0.5, rings: 2, spokes: 4 };
    const { lines } = generateGridGeometry(100, 100, config);
    expect(lines.length).toBeGreaterThan(0);
  });
